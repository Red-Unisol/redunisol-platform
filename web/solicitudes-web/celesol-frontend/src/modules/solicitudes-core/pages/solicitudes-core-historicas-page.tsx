...
import { useSolicitudesCoreHistoricasQuery } from "@/modules/solicitudes-core/hooks/use-solicitudes-core-historicas-query";
import {
...
const CORE_HISTORICAS_COLUMNS: SolicitudesTableColumn[] = [
  {
...
const CORE_HISTORICAS_LEFT_ACTIONS: SolicitudesToolbarAction[] = [];

const CORE_HISTORICAS_RIGHT_ACTIONS: SolicitudesToolbarAction[] = [
...
function getHistoricasCoreRowStyle(
...
export function SolicitudesCoreHistoricasPage() {
  ...
  const [columnFilters, setColumnFilters] = useState<SolicitudesTableFilters>(
    () => {
      ...
      return estadoFromUrl ? { estado: [estadoFromUrl] } : {};
    },
  );
  const isRefreshPendingRef = useRef(false);
  const { data, error, isFetching, isLoading, refetch } =
    useSolicitudesCoreHistoricasQuery({
  ...
  const tableRows = useMemo<SolicitudesTableRow[]>(() => {
    ...
    return data.map((item) => ({
      ...
      estadoCode: item.estadoActual.code ?? "",
  ...
  const filterOptions = useMemo(
    () =>
      buildSolicitudesTableFilterOptions(tableRows, CORE_HISTORICAS_COLUMNS),
    [tableRows],
  );
  ...
  const rightActions = useMemo(
    () =>
      CORE_HISTORICAS_RIGHT_ACTIONS.map((action) =>
        action.id === "refresh"
  ...
  return (
    ...
    >
      <SolicitudesToolbar
        leftActions={CORE_HISTORICAS_LEFT_ACTIONS}
        onAction={(actionId) => {
          if (actionId === "refresh") {
            void handleRefresh();
          }
      ...
      <div className="min-h-0 flex-1">
        {refreshState.isInitialLoading ? (
          ...
          <SolicitudesTable
            ...
            columns={CORE_HISTORICAS_COLUMNS}
            emptyMessage={
            ...
            rowStyle={getHistoricasCoreRowStyle}
            rows={filteredRows}
