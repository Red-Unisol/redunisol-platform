...
import type {
  ...
  AnalistaDashboardStats,
} from "../types";

export function getDashboardAnalistaStats(
  filters: AnalistaDashboardFilters,
): Promise<AnalistaDashboardStats> {
  ...
  return apiClient.get<AnalistaDashboardStats>(
    `/solicitudes/stats/analista${qs ? `?${qs}` : ""}`,
  );
}
