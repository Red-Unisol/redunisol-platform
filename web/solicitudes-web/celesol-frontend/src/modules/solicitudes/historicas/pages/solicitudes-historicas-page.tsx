...
import { useSolicitudesHistoricasQuery } from "@/modules/solicitudes/hooks/use-solicitudes-historicas-query";
...
const HISTORICAS_COLUMNS: SolicitudesTableColumn[] = [
  {
...
const HISTORICAS_LEFT_ACTIONS: SolicitudesToolbarAction[] = [
  { disabled: true, icon: ListFilterPlus, id: "atajo-1" },
  { icon: Calculator, id: "simulador" },
];

const HISTORICAS_RIGHT_ACTIONS: SolicitudesToolbarAction[] = [
...
function getHistoricasRowClassName(
...
export function SolicitudesHistoricasPage() {
  ...
  const { data, error, isLoading } = useSolicitudesHistoricasQuery();
  ...
  const filterOptions = useMemo(
    () => buildSolicitudesTableFilterOptions(tableRows, HISTORICAS_COLUMNS),
    [tableRows],
  );
  ...
  return (
    <article className="flex h-full min-h-0 flex-col overflow-hidden rounded-md border border-border bg-surface shadow-sm">
      <SolicitudesToolbar
        leftActions={HISTORICAS_LEFT_ACTIONS}
        onAction={(actionId) => {
        ...
        rightActions={HISTORICAS_RIGHT_ACTIONS}
      ...
      <div className="min-h-0 flex-1">
        {isInitialLoading ? (
          ...
          <SolicitudesTable
            ...
            rowClassName={getHistoricasRowClassName}
            rows={pageRows}
