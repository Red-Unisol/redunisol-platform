...
class ReconcileEdnaLanding implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        try {
            ...
            DB::table('edna_router_results')->where('id', $send->id)->whereIn('state', ['sending', 'accepted', 'unknown'])->update([
                'state' => $state, 'reason' => $reason, 'outgoing_message_id' => (string) $message['messageId'], 'updated_at' => now(),
            ]);
        } catch (Throwable) {
            throw new RuntimeException('Edna landing history unconfirmed; inspect the send ID.');
        }
    }

    public function failed(?Throwable $exception): void
    {
        DB::table('edna_router_results')->where('id', $this->sendId)->whereIn('state', ['sending', 'accepted', 'unknown'])
