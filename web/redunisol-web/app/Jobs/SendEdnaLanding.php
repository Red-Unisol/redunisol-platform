...
class SendEdnaLanding implements ShouldQueue
{
    use Queueable;
    ...
    {
        try {
            $send = DB::table('edna_router_results')->where('id', $this->sendId)->first();
            $flow = $send ? DB::table('edna_flow_sends')->find($send->flow_send_id) : null;
            if ($send && $flow) {
                foreach (['recipient', 'subject_id', 'cascade_id'] as $key) {
                    $send->{$key} = $flow->{$key};
                }
            }
            ...
            if ($send->state !== 'pending') {
                ...
                Queue::connection('edna')->push(new ReconcileEdnaLanding($send->id), '', 'edna');

                return;
            }
            if (! $flow || ! (new EdnaRouterResult)->eligible($flow)) {
                $this->closePending('cancelled', 'router_disabled');

                return;
            }
            if (CarbonImmutable::parse($send->response_received_at)->lt(now()->subHours(23))) {
            ...
            $claimed = DB::table('edna_router_results')->where('id', $send->id)->where('state', 'pending')->update([
            ...
            DB::transaction(function () use ($send, $state, $reason): void {
                DB::table('edna_router_results')->where('id', $send->id)->where('state', 'sending')->update([
                    'state' => $state, 'reason' => $reason,
                    'accepted_at' => $state === 'accepted' ? now() : null, 'updated_at' => now(),
                ]);
                if ($state !== 'rejected') {
                    Queue::connection('edna')->later(5, new ReconcileEdnaLanding($send->id), '', 'edna');
                }
            });
        } catch (Throwable) {
            throw new RuntimeException('Edna landing job failed; inspect the send ID.');
        }
    }

    private function closePending(string $state, string $reason): void
    {
        DB::table('edna_router_results')->where('id', $this->sendId)->where('state', 'pending')
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        DB::table('edna_router_results')->where('id', $this->sendId)->where('state', 'sending')
