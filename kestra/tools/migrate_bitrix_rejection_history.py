...
def receiver_stats(credential_file):
    """Read-only SSH health check; no runtime configuration or data is changed."""
    import paramiko
    text = Path(credential_file).read_text(encoding='utf-8-sig')
    block = text.split('===== SSH: VPS PRINCIPAL / DATTAWEB =====', 1)[1].split('=====', 1)[0]
    access = {k.strip(): v.strip() for line in block.splitlines() if '=' in line for k,v in [line.split('=', 1)]}
    if not all(access[key] in access['COMMAND'] for key in ('HOST','PORT','USER')):
        raise ValueError('SSH connection does not match the canonical command')
    connection = paramiko.SSHClient()
    connection.load_system_host_keys()
    try:
        connection.connect(access['HOST'], port=int(access['PORT']), username=access['USER'], password=access['PASSWORD'], look_for_keys=False, allow_agent=False, timeout=15)
        _, stdout, _ = connection.exec_command('cd /opt/bitrix-lead-receiver && python3 operate.py stats', timeout=25)
        if stdout.channel.recv_exit_status():
            raise ApiFailure('Receiver status command failed')
        return json.loads(stdout.read().decode())
    except ApiFailure:
        raise
    except Exception as exc:
        raise ApiFailure('Receiver read failed: ' + type(exc).__name__) from None
    finally:
        connection.close()
...
class Client:
    def __init__(self, credential_file):
        text = Path(credential_file).read_text(encoding='utf-8-sig')
        found = re.search(r'https://redunisol\.bitrix24\.es/rest/\d+/[^/\s\x22\x27]+/?', text)
        if not found:
            raise ValueError('Bitrix access missing')
        self.base = found.group(0).rstrip('/') + '/'
...
def main():
    ...
    parser.add_argument('mode', choices=['preflight','pilot','check-pilot','remaining'])
    ...
    parser.add_argument('--batch-interval-seconds', type=float, default=60)
    parser.add_argument('--night-window', action='store_true', help='Restrict remaining records to 22:00-06:00 Argentina')
    parser.add_argument('--queue-max-pending', type=int, default=125)
    args = parser.parse_args()
    if not 1 <= args.batch_size <= 50 or args.max_records < 1 or args.batch_interval_seconds < 0 or not 1 <= args.queue_max_pending <= 150:
        raise ValueError('Invalid limits')
    if args.mode == 'remaining':
        if not args.night_window:
            raise ValueError('Remaining inventory is authorized for night execution only')
        if not in_night_window():
            print(json.dumps({'status':'outside_night_window','at':utcnow(),'external_mutations':0}))
            return
    inventory = args.inventory_dir
    summary = read_json(inventory / 'summary.json')
    candidates = read_json(inventory / 'candidates.json')
    pilot = read_json(inventory / 'pilot-50.json')
    canonical = {row['ID']: row for row in candidates}
    if len(pilot) != 50 or len({row['ID'] for row in pilot}) != 50 or any(canonical.get(row['ID']) != row for row in pilot):
        raise ValueError('Pilot is not a unique approved subset')
    manifest = read_json(Path(__file__).parents[1] / 'automations/marketing-crm/bitrix/rejection-notifications/manifest.json')
    digest = hashlib.sha256((inventory / 'candidates.json').read_bytes()).hexdigest()
    validate_inventory(candidates, args.expected_sha256, digest, manifest, read_json(inventory / 'initial-counts.json'), summary['cutoff'])
    validate_guard(read_json(inventory / 'guard-before-migration.json'), summary['cutoff'])
    client = Client(args.credentials_file)
    journal = Journal(inventory / 'execution')
    live_reasons = client.call('crm.lead.userfield.list', {'filter': {'FIELD_NAME': REASON}})['result']
    recorded = read_json(inventory / 'initial-counts.json')['reason_fields']
    flatten = lambda fields: {(str(x['ID']),x['XML_ID'],x['VALUE']) for f in fields for x in f['LIST']}
    if flatten(live_reasons) != flatten(recorded):
        raise ValueError('Live reason catalog changed')
    live_stages = {row['STATUS_ID']: row for row in client.call('crm.status.list', {'filter': {'ENTITY_ID': 'STATUS'}})['result']}
    for source in manifest['sources']:
        current_stage = live_stages.get(source['source_stage'], {})
        if current_stage.get('SEMANTICS') != 'F' or current_stage.get('NAME') != source['reason_label']:
            raise ValueError('Live source stage catalog changed')
    if args.mode == 'preflight':
        before = communication_snapshot(client, [row['ID'] for row in pilot])
        path = journal.directory / 'pilot-before.json'
        if path.exists():
            raise ValueError('Pilot baseline already exists; preserve it')
        write_json(path, before)
        print(json.dumps({'pilot_leads': 50, 'linked_chats': len(before['last_message_ids']), 'baseline_saved': True}))
        return
    if args.mode == 'check-pilot':
        states, _ = journal.states()
        if any(states.get(row['ID']) not in ('verified','recovered') for row in pilot):
            raise ValueError('Pilot not fully verified')
        for offset in range(0,50,25):
            data=client.leads([r['ID'] for r in pilot[offset:offset+25]])
            if any(not all(norm(data.get(r['ID'],{}).get(k))==norm(v) for k,v in r['proposed'].items()) for r in pilot[offset:offset+25]):
                raise ValueError('Pilot fields changed after verification')
        after = communication_snapshot(client, [row['ID'] for row in pilot])
        write_json(journal.directory / 'pilot-after.json', after)
        result = compare_communications(read_json(journal.directory / 'pilot-before.json'), after)
        result.update({'checked_at': utcnow(), 'inventory_sha256': digest})
        write_json(journal.directory / 'pilot-validation.json', result)
        print(json.dumps(result))
        return
    if not args.execute:
        raise ValueError('Mutations require --execute')
    if not (journal.directory / 'pilot-before.json').exists():
        raise ValueError('Pilot communication baseline missing')
    if args.mode == 'remaining':
        validation = read_json(journal.directory / 'pilot-validation.json')
        if not validation['clean'] or validation['inventory_sha256'] != digest:
            raise ValueError('Pilot did not pass')
        rows = candidates
    else:
        rows = pilot
    states, _ = journal.states()
    rows = [row for row in rows if row['ID'] not in states][:args.max_records]
    lock = journal.directory / 'running.lock'
    with lock.open('x', encoding='utf-8') as stream:
        stream.write(str(os.getpid()))
    try:
        offset = 0
        while offset < len(rows):
            if args.mode == 'remaining' and not in_night_window():
                print(json.dumps({'status':'night_window_finished','at':utcnow()}), flush=True)
                break
            if args.mode == 'remaining':
                stats = receiver_stats(args.credentials_file)
                capacity = queue_capacity(stats, args.queue_max_pending)
                write_json(journal.directory / 'receiver-latest.json', {'at':utcnow(),**stats})
                if capacity < min(args.batch_size, len(rows)-offset):
                    print(json.dumps({'status':'waiting_for_receiver','at':utcnow(),'available_slots':capacity}), flush=True)
                    time.sleep(30)
                    continue
            start = time.monotonic()
            try:
                applied = apply_chunk(client, journal, rows[offset:offset+args.batch_size], in_night_window if args.mode == 'remaining' else None)
            except NightWindowClosed:
                print(json.dumps({'status':'night_window_finished','at':utcnow()}), flush=True)
                break
            offset += args.batch_size
            states, _ = journal.states()
            totals = dict(Counter(states.values()))
            write_json(journal.directory / 'progress.json', {'at': utcnow(), 'inventory_sha256': digest, 'total_candidates':len(candidates),'handled':len(states),'remaining':len(candidates)-len(states),'outcomes':totals})
            print(json.dumps({'batch_handled':len(applied),'total_handled':len(states),'outcomes':totals}), flush=True)
            if offset < len(rows):
                time.sleep(max(0,args.batch_interval_seconds-(time.monotonic()-start)))
    finally:
        lock.unlink()
...
if __name__ == '__main__':
    try:
        main()
    except (ValueError, ApiFailure) as exc:
        print(f'Stopped: {exc}', file=sys.stderr)
        sys.exit(1)
    except Exception as exc:
        print(f'Stopped: {type(exc).__name__}', file=sys.stderr)
        sys.exit(1)
