...
def prepare_warmup_batch() -> tuple[
    CoreConfig,
    CredixConfig,
    dict[str, Any],
    list[CoreSolicitud],
    ...
    # Etapa compartida de pre-check: carga configuracion, releva solicitudes del dia y arma el lote candidato.
    ...
    # Primero consulta Vimarx para evitar entrar a CredixSA cuando no hay trabajo real.
    solicitudes = fetch_today_solicitudes(core_config, today)
    solicitudes = complete_missing_cuils(core_config, solicitudes)
    # Filtra lo ya cubierto por el indice diario antes de ejecutar la parte costosa.
    candidates = select_candidates(solicitudes, daily_index, max_per_run)

    return core_config, credix_config, daily_index, solicitudes, candidates


def run_precheck() -> dict[str, Any]:
    # Ejecuta solo la deteccion liviana para saber si hay candidatos reales antes de entrar al paso costoso.
    _core_config, _credix_config, _daily_index, solicitudes, candidates = (
        prepare_warmup_batch()
    )
    return {
        "ok": True,
        "has_candidates": len(candidates) > 0,
        "solicitudes_count": str(len(solicitudes)),
        "candidate_count": str(len(candidates)),
        "skipped_count": str(max(0, len(solicitudes) - len(candidates))),
        "error": "",
    }


def run_warmup() -> dict[str, Any]:
    # Reusa la preparacion compartida para poder separar luego el pre-check del procesamiento pesado.
    _core_config, credix_config, daily_index, solicitudes, candidates = (
        prepare_warmup_batch()
    )
