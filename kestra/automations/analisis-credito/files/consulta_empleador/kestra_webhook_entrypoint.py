...
def main() -> int:
    ...
    try:
        ...
        result = consultar_empleador(request, config)
        return _finalize_execution(build_output_payload(result), exit_code=0)

    except InvalidRequestError as exc:
        logger.warning("Solicitud invalida de consulta empleador: %s", exc)
        result = build_error_result(request, str(exc), status="invalid_request")
        return _finalize_execution(build_output_payload(result), exit_code=0)

    except ConfigurationError as exc:
        logger.error(
            "Configuracion invalida del flow consulta_empleador: %s", exc, exc_info=True
        )
        ...
        logger.error("Error tecnico al consultar PYPDatos: %s", exc, exc_info=True)
        result = build_error_result(request, str(exc), status=exc.status)
        return _finalize_execution(build_output_payload(result), exit_code=1)

    except Exception as exc:
        logger.error("Fallo inesperado en consulta_empleador: %s", exc, exc_info=True)
        result = build_error_result(request, str(exc), status="technical_error")
        return _finalize_execution(build_output_payload(result), exit_code=1)
