...
def build_monthly_comparison(workbook, months, reference_rows):
    """Historico compacto de tiempos y sus objetivos."""
    ...
    def table(row, title, metric):
        ...
        ws.conditional_formatting.add(f"D{start}:D{end}", ColorScaleRule(
            start_type="num", start_value=-0.10, start_color=green,
            mid_type="num", mid_value=0, mid_color=yellow,
            end_type="num", end_value=0.10, end_color=red))
