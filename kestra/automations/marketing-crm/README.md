...
- `flows/bitrix24_lead_prefill.yaml`: backfill de leads `NEW` con ARCA, CredixSA, Vimarx y BCRA.
- `flows/bitrix24_lead_classification.yaml`: flow interno de clasificacion por `lead_id`.
- `flows/bitrix24_lead_won_deal_webhook.yaml`: receptor de `ONCRMLEADUPDATE`; clasifica `PRECLASIFICACION` y crea negociaciones desde `RESULTADO GANADO`.
...
- El webhook de formulario solo crea contacto y lead en `NEW`; no consulta proveedores ni toma decisiones comerciales.
- El prefill no considera ownership. Reintenta hasta tres veces y luego mueve el lead a `PRECLASIFICACION`, incluso si el enriquecimiento quedo parcial.
- `ONCRMLEADUPDATE` clasifica un lead en `PRECLASIFICACION` solamente cuando `Motor decision comercial = Kestra`.
